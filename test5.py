...
AUTO_RESET_DELAY = 3000  # Thời gian dừng hình kết quả (ms) trước khi reset
...
def process_logic(frame, model, cam_idx, state_dict, collected_items):
    ...
    results = model.predict(frame, conf=0.5, imgsz=640, verbose=False, device=DEVICE)[0]
...
def main():
    ...
    print("--- Running... System will AUTO-RESET when all cams passed ---")
    ...
    while running:
        ...
        # --- LOGIC TỰ ĐỘNG HIỂN THỊ VÀ RESET ---
        if all_passed:
            # 1. Vẽ màn hình kết quả PASSED
            final_img = show_final_result(combined, True, [])
            cv2.imshow("HE THONG KIEM TRA PACKAGING", final_img)
            
            # 2. Dừng hình trong 3 giây (để công nhân nhìn thấy màu xanh)
            cv2.waitKey(AUTO_RESET_DELAY)
            
            # 3. Reset hệ thống để bắt đầu vòng lặp mới
            states = {}
            collected_items.clear()
            print(f"--- [AUTO] Completed cycle. Resetting for next item... ---")
            
            # Bỏ qua phần còn lại của vòng lặp để bắt đầu lại ngay
            continue
